...
| drift-lke-update-strategy | `e2e/drift-lke-update-strategy/chainsaw-test.yaml` | `all`, `nightly`, `category.drift` | Drift-triggered replacement when `LinodeNodeClass.spec.lkeUpdateStrategy` changes for workload pinned to a drift-specific NodePool | Ensures NodeClass drift rotates capacity without relying on mutable default-pool behavior |

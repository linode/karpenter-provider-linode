...
func NewController(
	...
	if resolvedOpts.LinodeNodeClassProvider == nil {
		resolvedOpts.LinodeNodeClassProvider = &defaultNodeClassProvider{kubeClient: kubeClient}
	}
	return &Controller{
...
func (c *Controller) Reconcile(ctx context.Context, node *corev1.Node) (reconcile.Result, error) {
	...
	nodeClass, err := c.GetLinodeNodeClass(ctx, nodeClaim)
	if err != nil {
		return reconcile.Result{}, fmt.Errorf("failed to get Linodenodeclass, %w", err)
	}
	if err := c.instancetypeProvider.UpdateInstanceTypeCapacityFromNode(ctx, node, nodeClass); err != nil {
...
type LinodeNodeClassProvider interface {
	GetLinodeNodeClass(context.Context, *karpv1.NodeClaim) (*v1.LinodeNodeClass, error)
}
...
func (p *defaultNodeClassProvider) GetLinodeNodeClass(ctx context.Context, nodeClaim *karpv1.NodeClaim) (*v1.LinodeNodeClass, error) {
...
func WithLinodeNodeClassProvider(provider LinodeNodeClassProvider) option.Function[ControllerOpts] {
	return func(c *ControllerOpts) {
		c.LinodeNodeClassProvider = provider
	}
}

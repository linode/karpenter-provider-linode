...
var _ = Describe("InstanceType", func() {
	It("should update instance type date with response from the ListTypes API", func() {
		linodeInstanceTypes := fake.MakeInstances()
		linodeOfferings := fake.MakeInstanceOfferings(linodeInstanceTypes)
		linodeEnv.LinodeAPI.ListTypesOutput.Set(&linodeInstanceTypes)
		...
		Expect(instanceTypes).To(HaveLen(len(linodeInstanceTypes)))
		for _, it := range instanceTypes {
			Expect(lo.ContainsBy(linodeInstanceTypes, func(i linodego.LinodeType) bool { return i.ID == it.Name })).To(BeTrue())
		}
	})
	It("should update instance type offering date with response from the ListRegionsAvailability API", func() {
		...
		Expect(len(instanceTypes)).To(BeNumerically("==", len(linodeInstanceTypes)))
	...
	It("should not update instance type date with response from the ListTypes API is empty", func() {
		linodeEnv.LinodeAPI.ListTypesOutput.Set(&[]linodego.LinodeType{})
		linodeEnv.LinodeAPI.ListRegionsAvailabilityOutput.Set(&[]linodego.RegionAvailability{})
		ExpectSingletonReconciled(ctx, controller)

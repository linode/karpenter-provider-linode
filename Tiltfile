...
local_resource(
    'karpenter',
    cmd="helm upgrade --install --namespace kube-system --create-namespace karpenter charts/karpenter --set settings.clusterName=${CLUSTER_NAME} --set apiToken=${LINODE_TOKEN} --set controller.image.repository=${KO_DOCKER_REPO} --set-string controller.image.tag=${IMAGE_TAGS:-$(git rev-parse --abbrev-ref HEAD)} --set settings.batchMaxDuration=10s --set settings.batchIdleDuration=1s --wait --debug --timeout 10m",
    env={'LINODE_TOKEN': os.getenv("LINODE_TOKEN"), 'CLUSTER_NAME': os.getenv("CLUSTER_NAME"), 'KO_DOCKER_REPO': os.getenv("KO_DOCKER_REPO", "docker.io/linode/karpenter-provider-linode"), 'IMAGE_TAGS': os.getenv("IMAGE_TAGS", "")},
    labels=["karpenter"],
